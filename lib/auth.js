    ...
    let strategy = new Strategy({
            ...
            try {
                ...
                if (user) {
                    ...
                    let privileges = rolePrivileges.map(rp => privs.privileges.find(p => p.key == rp.permission));
        ...
        checkRoles: (...expectedRoles) => {
            return (req, res, next) => {
                ...
                if  (i >= expectedRoles.length) {
                    let response = Response.errorResponse(res, new CustomError(HTTP_CODES.UNAUTHORIZED, "Need Permission", "Need Permission"));
